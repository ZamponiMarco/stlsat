...
fn max_jump_one_returns_max() {
    ...
    assert_eq!(node.calculate_k_star(), i32::MAX);
    assert_eq!(node.calculate_k_star(), i32::MAX);
}
...
fn gap_completeness_computed() {
    let mut node = Node::from_operands(vec![
        ...
        NodeFormula::from(Formula::f(Interval { lower: 0, upper: 6 }, prop("a"))).with_marked(true),
    ]);
    node.current_time = 0;
    assert_eq!(node.calculate_k_star(), 5);
}

#[test]
fn gap_soundness_computed() {
    let mut node = Node::from_operands(vec![
        NodeFormula::from(Formula::g(Interval { lower: 0, upper: 6 }, prop("a"))).with_marked(true),
